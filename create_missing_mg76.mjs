...
async function createMissing() {
  console.log("🆕 Creating missing products for mg76\n");
  ...
  try {
    ...
    const missingProducts = MG76_STOCK_DATA.filter(([name]) => !existingNames.has(name));
    ...
    for (const [productName, quantity] of missingProducts) {
      ...
      const { data: newProduct, error: createError } = await supabase
        ...
        .insert({
          ...
          stock_reference: "STOCKS MG 76", // Set stock_reference
        })
        .select("id")
        .single();
