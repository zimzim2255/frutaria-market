...
async function createMissing() {
  console.log("🆕 Creating missing products for mg813\n");
  ...
  try {
    ...
    const missingProducts = MG813_STOCK_DATA.filter(([name]) => !existingNames.has(name));
    ...
    for (const [productName, quantity] of missingProducts) {
      // Create product with stock_reference
      const { data: newProduct, error: createError } = await supabase
        ...
        .insert({
          ...
          stock_reference: "STOCKS MG 813", // Set stock_reference
        })
        .select("id")
        .single();
