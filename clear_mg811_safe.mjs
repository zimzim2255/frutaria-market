...
async function clearStocks() {
  console.log("🗑️  CLEARING MG811 STOCKS (store_stocks + products.quantity_available)\n");
...
i
    if (!stores || stores.length === 0) {
      console.error("❌ Could not find mg811 store");
      process.exit(1);
    }
    ...
    // Step 1: Delete all store_stocks for mg811
    ...
    // Step 2: Reset all products.quantity_available to 0 for mg811 (NOT delete products)
    ...
    if (remainingStocks === 0 && (productCheck?.length || 0) === 0) {
      console.log("\n✅ COMPLETE! store_stocks cleared and products reset for mg811");
    } else {
