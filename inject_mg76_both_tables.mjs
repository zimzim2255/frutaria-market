...
async function injectWithNoDuplication() {
  console.log("💉 MG76 INJECTION WITH DUPLICATION PREVENTION\n");
  console.log(`📦 Products to inject: ${MG76_STOCK_DATA.length}\n`);

  try {
    // 1. Get all existing products for mg76
    console.log("📋 Step 1: Fetching existing mg76 products...");
    ...
    // 2. Get all existing store_stocks for mg76
    console.log("📋 Step 2: Fetching existing mg76 store_stocks...");
    ...
    for (let i = 0; i < MG76_STOCK_DATA.length; i++) {
      const [productName, quantity] = MG76_STOCK_DATA[i];

      // Show progress
      if ((i + 1) % 10 === 0) {
        console.log(`├─ ${i + 1}/${MG76_STOCK_DATA.length}`);
      }
